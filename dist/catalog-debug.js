...
*/
define("widget/catalog/1.0.0/catalog-debug", [ "widget-debug" ], function(require, exports, module) {
    ...
    return Widget.extend({
        attrs: {
            ...
            // 用于显示的类目文本字段
            response: null,
            // 后端返回的数据，如果有response的情况下，url这个就会禁用掉
            url: null,
            // 请求的接口地址
            param: {},
            // 请求的参数
        ...
        getData: function(options) {
            var This = this, Url = this.get("url"), param = this.get("param"), dealData = function(res) {
                var olist = [];
        ...
        select: function(ele) {
            ...
            var ol = ele.parents("ol"), id = ele.attr("data-id"), fid = ol.attr("data-fid"), clsbox = ol.parent(".catalog-tier"), isleaf = ele.attr("leaf") == "true";
        ...
        checkValue: function(curEle, curNodeId, fId) {
            var catalogbox = curEle.parents(".catalog-tier"), catalogIndex = catalogbox.index(), _catalogbox = curEle.parents(".catalog-tier").prev(".catalog-tier");
            if (catalogIndex > 0) {
                var curLi = _catalogbox.find('li[data-id="' + fId + '"]'), //oitem     = arguments.callee( curLi, fId, curLi.parents('ol').attr('data-fid') );
